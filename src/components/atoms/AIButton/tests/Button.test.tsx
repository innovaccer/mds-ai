...
import AIButton, { ButtonProps } from '../index';
...
describe('Button', () => {
  ...
  it('renders with default props', () => {
    const { getByTestId } = render(<AIButton {...defaultProps} />);
  ...
  it('renders with a `primary` appearance', () => {
    const { getByTestId } = render(<AIButton {...defaultProps} appearance="primary" />);
    const button = getByTestId('DesignSystem-AI-Button');
  ...
  it('renders a custom className', () => {
    ...
    const { getByTestId } = render(<AIButton {...defaultProps} className={customClassName} />);
    const button = getByTestId('DesignSystem-AI-Button');
    expect(button).toHaveClass(customClassName);
  });

  it('is disabled when `disabled` prop is true', () => {
    const { getByTestId } = render(<AIButton {...defaultProps} disabled />);
    const button = getByTestId('DesignSystem-AI-Button');
